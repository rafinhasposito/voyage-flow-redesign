...
import { getTravelState, generateSmartItinerary, Attraction, ItineraryDay, RecommendedExperience, TravelState, UserProfile } from "@/utils/travelState";
...
export default function TripWorkspace() {
  ...
  const saveTravelState = async (newState: TravelState) => {
    if (!tripId) return;
    try {
      await TripRepository.updateTripOnboarding(tripId, { itinerary: newState.itinerary });
    } catch(err) {
    ...
    <div className="min-h-screen bg-[#FAF8F5] text-[#1C1E21] flex flex-col">
      ...
      <header className="border-b border-[#EAE6DF] bg-[#FAF8F5]/80 backdrop-blur sticky top-0 z-50">
        <div className="mx-auto flex max-w-[1240px] items-center justify-between px-6 py-4 md:px-10">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium md:flex">
            <Link to={`/viagens/${tripId}/roteiro`} className="text-[#C5A85C]">Meu Roteiro</Link>
            <Link to="/app/catalog" className="text-slate-600 hover:text-[#0D0E10]">Explorar Atrações</Link>
            <Link to={`/viagens/${tripId}/carteira`} className="text-slate-600 hover:text-[#0D0E10]">Orçamento & Gastos</Link>
          </nav>
          <Link
            to="/onboarding"
      ...
      <main className="flex-1 mx-auto w-full max-w-[1240px] px-6 py-8 md:px-10 grid gap-8 lg:grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)]">
        ...
        <div className="space-y-6">
          ...
          {/* Map Card */}
          <div className="bg-white rounded-3xl border border-[#EAE6DF] p-2 space-y-6 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.03)] h-[400px]">
             <MapLibreMap attractions={currentDayData ? currentDayData.attractions : []} />
          </div>
